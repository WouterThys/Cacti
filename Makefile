...
INSTALLERS_BUILD_DIR=$(RELEASE_DIR)\installers
...
clean:
	...
	if not exist "$(INSTALLERS_BUILD_DIR)" mkdir $(INSTALLERS_BUILD_DIR)
...
build_client:
	...
	xcopy /s /e ".NET\CactiService\CactiClient\bin\Release\net7.0-windows\" $(CLIENT_BUILD_DIR) /Y

CactiClient.exe: build_client
...
build_server:
	...
	xcopy /s /e ".NET\CactiService\CactiService\bin\Release\net7.0\" $(SERVER_BUILD_DIR) /Y
